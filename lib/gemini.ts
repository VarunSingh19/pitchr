...
// These train Gemini on the exact tone, length,
...
/**
 * Create a Gemini client instance for a specific user's API key.
 * No more global singleton — each user gets their own client.
 */
...
export async function generateEmailBody(
  ...
  const model = genAI.getGenerativeModel({
    model: modelName,
    systemInstruction: `You are an expert cold email copywriter who has written thousands of 
...
reference examples provided and match their tone, structure, and length exactly.`,
  ...
  const prompt = `Study these reference cold emails carefully — they show the EXACT tone, structure, length, and style required:
...
/** Generate a compelling subject line using Gemini */
export async function generateSubjectLine(
  ...
  const prompt = `Study these subject line examples:
